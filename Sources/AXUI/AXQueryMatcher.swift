...
public struct AXQueryMatcher {
    ...
    /// Direct field conditions and logical operators are combined with AND semantics.
    public static func matches(element: AXElement, query: AXQuery, allElements: [AXElement]) -> Bool {
        // 1. Match direct fields first
        if !matchesDirectFields(element: element, query: query, allElements: allElements) {
            return false
        }

        // 2. Logical operators (AND-combined with direct fields)
        if let andQueries = query.andQueries {
            if !andQueries.allSatisfy({ matches(element: element, query: $0.value, allElements: allElements) }) {
                return false
            }
        }

        if let orQueries = query.orQueries {
            if !orQueries.contains(where: { matches(element: element, query: $0.value, allElements: allElements) }) {
                return false
            }
        }

        if let notQuery = query.negatedQuery {
            if matches(element: element, query: notQuery.value, allElements: allElements) {
                return false
            }
        }
    ...
    /// Match direct (non-logical) fields of a query against an element
    private static func matchesDirectFields(element: AXElement, query: AXQuery, allElements: [AXElement]) -> Bool {
        ...
        if let descriptionRegex = query.descriptionRegex {
            guard let elementDescription = element.description else {
                return false
            }
            do {
                ...
                if regex.firstMatch(
                    ...
                    options: [],
                    range: NSRange(location: 0, length: elementDescription.utf16.count)
                ) == nil {
                    return false
                }
            } catch {
        ...
        if let identifierRegex = query.identifierRegex {
            guard let elementIdentifier = element.identifier else {
                return false
            }
            do {
                let regex = try NSRegularExpression(pattern: identifierRegex, options: [])
                if regex.firstMatch(
                    in: elementIdentifier,
                    options: [],
                    range: NSRange(location: 0, length: elementIdentifier.utf16.count)
                ) == nil {
                    return false
                }
            } catch {
        ...
        // Position matching (Double precision)
        ...
        // Size matching (Double precision)
        ...
        // Spatial matching using Double precision from position/size directly
        if let boundsContains = query.boundsContains {
            guard let position = element.position, let size = element.size else {
                return false
            }
            let contains = boundsContains.x >= position.x &&
                           boundsContains.y >= position.y &&
                           boundsContains.x <= position.x + size.width &&
                           boundsContains.y <= position.y + size.height
        ...
        if let boundsIntersects = query.boundsIntersects, boundsIntersects.count == 4 {
            ...
            let qx = boundsIntersects[0]
            let qy = boundsIntersects[1]
            let qw = boundsIntersects[2]
            let qh = boundsIntersects[3]
            // Two rectangles intersect if neither is completely to the left/right/above/below the other
            let intersects = !(position.x >= qx + qw ||
                              qx >= position.x + size.width ||
                              position.y >= qy + qh ||
                              qy >= position.y + size.height)
