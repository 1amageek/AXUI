...
@Test func testAccessibilityPermissionCheck() throws {
    ...
    if !hasPermissions {
        ...
        print("ℹ️  Note: This is informational only. Tests requiring permissions will be skipped.")
    }

    // This test always passes - it just checks if the permission check method works
    #expect(Bool(hasPermissions) || Bool(!hasPermissions)) // Always true
}
...
@Test func testPermissionRequestFlow() throws {
    ...
    do {
        ...
        if !initialPermissions {
            print("⚠️  Dump succeeded without permissions - unexpected but not a failure")
        }
    } catch AXDumperError.accessibilityPermissionDenied {
        #expect(!initialPermissions, "Should only get permission error when permissions are not granted")
        print("✅ Correctly threw permission denied error")
    } catch AXDumperError.tooManyElements(let found, let limit) {
        #expect(initialPermissions, "Should only get too many elements error when permissions are granted")
